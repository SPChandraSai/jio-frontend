    ...
    return (
        <header className="bg-[#0d0e10] py-4 w-full fixed top-0 z-50 border-b-2 border-b-gray">
            <div className="lg:mx-auto mx-2 lg:px-4 flex items-center text-nowrap">
                ...
                <nav className="lg:flex lg:space-x-4 space-x-0">
                    {navLinks.map((tab) => (
                        <Link
                            href={tab.href}
                            key={tab.key}
                            className={`px-1 py-2 font-medium text-[#b6b8b8] hover:text-white ${activeTabKey===tab.key ? "border-b-2 border-pink-500 text-white" : ""}`}
                        >
                            {tab.name}
                        </Link>
                    ))}
                ...
                <div className="flex items-center justify-end w-full">
                    <div className="rounded-3xl border lg:flex justify-center items-center px-4 gap-2">
